  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 text-sm font-medium text-emerald-600">
        <CheckCircle className="size-4" /> {t('delivery.addedToCart')}
      </div>
      <div className="mt-3 flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">{t('delivery.qtyLine', { qty: quantity, price: `${CURRENCY}${lineTotal}` })}</p>
        </div>
      </div>
      <div className="mt-3 flex gap-2">
        <Button asChild size="sm" className="flex-1 gap-1.5" onClick={dismissLastAdded}>
          <Link to="/cart"><ShoppingBag className="size-3.5" /> {t('delivery.viewCart', { count: cartCount })}</Link>
        </Button>
